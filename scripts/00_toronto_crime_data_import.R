...
### Graphing robbery rates of 16 neighbourhoods in the downtown core ###
toronto_crime %>%
...
### Finding the neighbourhoods with the highest robbery average and plotting the crime numbers over time ###
...
compare <- merge(x = toronto_crime_all, y = toronto_crime_pop, by = "Neighbourhood", all = TRUE) # merging the population and crime dataframes
compare %>% # plotting the number of crimes with their population
  ggplot(aes(x = Neighbourhood, y = Number_of_Crimes, fill = Population)) +
